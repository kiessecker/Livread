 ...
 ***********/
 	 ...
 	 **/
 	public $ebookData;
	public $livreadTocCount=0;
	public $livreadTocNames=array();
	public $livreadTocHrefs=array();
	...
	private function recurseNestedNav($xmlObj,$depth=0) {
		foreach($xmlObj->children() as $child) {
//			echo "<br><strong>".$child->getName()."</strong>";
			if($child->getName()=="navPoint") {
//				print_r($this->getTag($child,"text")); 
//				print_r($this->getTag($child,"content"));

// print_r($this->getTag($child,"text"));
 
				foreach($this->getTag($child,"text") as $value) {
//					echo "here!";

					$this->livreadTocCount=$this->livreadTocCount+1;
					$this->livreadTocNames[]=$value[0];
				}
				foreach($this->getTag($child,"content") as $value) {
//					echo $this->optAttributeExist($value, "src");
					$this->livreadTocHrefs[]=$this->optAttributeExist($value, "src");
				}
			}
//   	 		echo str_repeat('-',$depth).">".$child->getName().": ";
   	 		$this->recurseNestedNav($child,$depth+1);
  		}
	}
	...
	private function loadTOC(){
		if(isset($this->ebookData->toc)){
			...
			$xml = simplexml_load_string($contents);
//			print_r($xml);
			$this->recurseNestedNav($xml);
//			print_r($this->livreadTocHrefs);
//			print_r($this->livreadTocNames);
			$names = $this->livreadTocNames;
			$href = $this->livreadTocHrefs;


			if($this->livreadTocCount==0) {
				// that is not complex nested toc file, so we still have no data after all
			...
			// to prevent duplicates while running furiously through toc
			$alreadyCreatedToc=array();

			for($x=0; $x < count($names);$x+=1){
				$names[$x]=(string)$names[$x];
				$href[$x]=(string)$href[$x];

				if(!in_array(md5($names[$x]."-".$href[$x]),$alreadyCreatedToc)) {
					$ch = new tocItem();
					$ch->name = $names[$x];
					$ch->fileName = $href[$x];
					array_push($finToc, $ch);
				}
				$alreadyCreatedToc[]=md5($names[$x]."-".$href[$x]);
	...
	private function readEpubFile($zipFile, $fileLocation){
		...
		$fileLocationAlt=ereg_replace("^\./","",$fileLocation);
		$ext = substr($zipFile, strrpos($zipFile, '.') + 1);
		if($ext == "epub" && is_file($zipFile)){
			$zip = zip_open($zipFile);
			$contents = "";
			if ($zip) {
			  while ($zip_entry = zip_read($zip)) {
			    	if(((zip_entry_name($zip_entry) == $fileLocation) || (zip_entry_name($zip_entry) == $fileLocationAlt)) && zip_entry_open($zip, $zip_entry))	{
		...
		if(is_a($epub, 'ebookData')){
			...
			if(!file_exists($epub)){
				...
				if($ext == "epub"){
					...
					trigger_error("You must input a file with the extension epub", E_USER_ERROR);
				}
			}
		}else{
