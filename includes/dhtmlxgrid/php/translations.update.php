...
function add_row(){
	...
	$sql = 	"INSERT INTO notes (notes_author,notes_bookname,notes_date,note_content,username)
			VALUES ('".addslashes(utf8_decode($_GET["c0"]))."',
					'".addslashes(utf8_decode($_GET["c1"]))."',
					'".addslashes(utf8_decode($_GET["c2"]))."',
					'".addslashes(utf8_decode($_GET["c3"]))."',
...
function update_row(){
	$sql = 	"UPDATE notes SET notes_author='".addslashes(utf8_decode($_GET["c0"]))."',
				notes_bookname=		'".addslashes(utf8_decode($_GET["c1"]))."',
				notes_date=		'".addslashes(utf8_decode($_GET["c2"]))."',
				note_content=		'".addslashes(utf8_decode($_GET["c3"]))."' 
...
function delete_row(){
	...
	$d_sql = "DELETE FROM translations WHERE translation_id=".$_GET["gr_id"]." AND 
		translation_username='".addslashes($_SESSION["username"])."'";
